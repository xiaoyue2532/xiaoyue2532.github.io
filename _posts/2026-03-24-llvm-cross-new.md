...
我的 arm 目标平台是 RK3506。这是一个 cortex-a7 核心的 armv7 处理器。
若不指定 march，默认是面向古早的 arm 架构。若面向 armv7开发时不确定 mtune 取值，可以填 generic-armv7-a。
...
compiler-rt 的安装位置有误 需要手动调整目录结构

```shell
cp -rf $(pwd)/_install/llvmtools/arm-unknown-linux-musleabihf/lib/linux \
    /llvmtools/lib/clang/21/lib/
rm -rf $(pwd)/_install/llvmtools/arm-unknown-linux-musleabihf/lib/linux

cp -rf $(pwd)/_install/llvmtools
    /
```
